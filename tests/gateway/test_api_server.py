...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_disabled_toolsets(self, monkeypatch):
        captured = {}
        ...
        monkeypatch.setattr(
            "gateway.run._resolve_runtime_agent_kwargs",
            lambda: {"provider": "openai-codex", "base_url": None, "api_mode": "codex_responses"},
        )
        monkeypatch.setattr("gateway.run._resolve_gateway_model", lambda: "gpt-5.5")
        monkeypatch.setattr(
            "gateway.run._load_gateway_config",
            lambda: {"agent": {"disabled_toolsets": ["todo"]}, "checkpoints": {"enabled": False}},
        )
        monkeypatch.setattr(
            "gateway.run.GatewayRunner._load_reasoning_config", staticmethod(lambda model="": None))
        monkeypatch.setattr("gateway.run.GatewayRunner._load_fallback_model", staticmethod(lambda: None))
        monkeypatch.setattr("hermes_cli.tools_config._get_platform_tools", lambda *_: {"todo", "search"})
        ...
        assert sorted(captured["enabled_toolsets"]) == ["search", "todo"]

    def test_create_agent_disabled_toolsets_none_when_not_configured(self, monkeypatch):
        captured = {}
        ...
        monkeypatch.setattr("gateway.run._resolve_gateway_model", lambda: "gpt-5.5")
        monkeypatch.setattr(
            "gateway.run._load_gateway_config", lambda: {"checkpoints": {"enabled": False}})
        monkeypatch.setattr(
            "gateway.run.GatewayRunner._load_reasoning_config", staticmethod(lambda model="": None))
        monkeypatch.setattr("gateway.run.GatewayRunner._load_fallback_model", staticmethod(lambda: None))
        monkeypatch.setattr("hermes_cli.tools_config._get_platform_tools", lambda *_: {"search"})
